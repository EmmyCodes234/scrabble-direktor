...
const StandingsTable = ({ players, recentResults, onSelectPlayer }) => {
  ...
  const CardView = () => (
    <div className="grid gap-4 p-4">
      {sortedPlayers.map((player) => {
        const lastGame = getLastGameStatus(player);
        return (
          <div key={player.id} className="glass-card p-4 hover:shadow-glow hover:border-primary/30 transition-smooth touch-target cursor-pointer" onClick={() => onSelectPlayer(player)}>
            ...
            <div className="text-sm text-muted-foreground">{getRecordDisplay(player)}</div>
